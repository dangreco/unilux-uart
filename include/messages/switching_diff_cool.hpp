...
/// @file switching_diff_cool.hpp
/// @brief Typed WMMM message carrying the cooling-mode switching differential.
///
/// The SwitchingDiffCool message (id @c 0x59, thermostat parameter P07) carries
...
/// @brief Cooling-mode switching differential (WMMM message id @c 0x59).
class SwitchingDiffCool : public unilux::Message {

public:
  /// WMMM message id for this message.
  static constexpr uint8_t ID = 0x59;
  ...
  SwitchingDiffCool() = default;
  explicit SwitchingDiffCool(float value) : value(value) {}
  ...
  std::string to_string() const override {
    char buf[40];
    std::snprintf(buf, sizeof(buf), "SwitchingDiffCool(%.1f)",
                  static_cast<double>(value));
    return buf;
  }

  /// @brief Decode a SwitchingDiffCool message from a generic WMMM frame.
  ...
  static std::optional<SwitchingDiffCool> decode(const Frame &frame) {
    if (frame.payload.size() != PAYLOAD_SIZE) {
      return std::nullopt;
    }

    SwitchingDiffCool msg;
