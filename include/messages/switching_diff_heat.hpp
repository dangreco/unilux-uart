...
/// @file switching_diff_heat.hpp
/// @brief Typed WMMM message carrying the heating-mode switching differential.
///
/// The SwitchingDiffHeat message (id @c 0x58, thermostat parameter P06) carries
/// a single temperature in a 4-byte payload: a big-endian 16-bit fixed-point
/// value in tenths of a degree Celsius (bytes 0..1), then two zero bytes.
/// Typical range is 2.0..4.0 °C.
///
/// @code
// clang-format off
/// +----------------+----------------+
...
/// @brief Heating-mode switching differential (WMMM message id @c 0x58).
class SwitchingDiffHeat : public unilux::Message {

public:
  /// WMMM message id for this message.
  static constexpr uint8_t ID = 0x58;
  ...
  /// The switching differential, in degrees Celsius.
  float value{};

  SwitchingDiffHeat() = default;
  explicit SwitchingDiffHeat(float value) : value(value) {}
  ...
  std::string to_string() const override {
    char buf[40];
    std::snprintf(buf, sizeof(buf), "SwitchingDiffHeat(%.1f)",
                  static_cast<double>(value));
    return buf;
  }

  /// @brief Decode a SwitchingDiffHeat message from a generic WMMM frame.
  ...
  static std::optional<SwitchingDiffHeat> decode(const Frame &frame) {
    if (frame.payload.size() != PAYLOAD_SIZE) {
      return std::nullopt;
    }

    SwitchingDiffHeat msg;
