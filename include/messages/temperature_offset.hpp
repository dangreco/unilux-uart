...
/// @file temperature_offset.hpp
/// @brief Typed WMMM message carrying the room-temperature calibration offset.
///
/// The TemperatureOffset message (id @c 0x56, thermostat parameter P05) carries
/// a single signed temperature in a 4-byte payload: a big-endian 16-bit
/// fixed-point value in tenths of a degree Celsius (bytes 0..1), then two zero
/// bytes. Typical range is -5.0..+5.0 °C.
///
/// @code
// clang-format off
/// +----------------+----------------+
...
/// @brief Room-temperature calibration offset (WMMM message id @c 0x56).
class TemperatureOffset : public unilux::Message {

public:
  /// WMMM message id for this message.
  static constexpr uint8_t ID = 0x56;
  ...
  /// The offset, in degrees Celsius (may be negative).
  float value{};

  TemperatureOffset() = default;
  explicit TemperatureOffset(float value) : value(value) {}
  ...
  std::string to_string() const override {
    char buf[40];
    std::snprintf(buf, sizeof(buf), "TemperatureOffset(%.1f)",
                  static_cast<double>(value));
    return buf;
  }

  /// @brief Decode a TemperatureOffset message from a generic WMMM frame.
  ...
  static std::optional<TemperatureOffset> decode(const Frame &frame) {
    if (frame.payload.size() != PAYLOAD_SIZE) {
      return std::nullopt;
    }

    TemperatureOffset msg;
